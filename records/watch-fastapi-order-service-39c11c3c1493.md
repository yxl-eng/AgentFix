...
- Incident：`watch-fastapi-order-service-39c11c3c1493`
...
- 根因类型：代码缺陷
- 风险等级：中
- 摘要：在取消订单流程中，代码试图访问旧订单（来自遗留导入器）的 `inventory_hold` 字段，但该字段不存在。根据 `app/repository.py` 中的注释，`ord-200` 是一个已支付的旧订单，没有 `inventory_hold` 字段。错误发生在 `app/services.py` 的 `_release_inventory_hold` 方法中，该方法假设所有订单都有此字段，并在执行状态检查之前被调用。
...
## 证据
- service=fastapi-order-service env=local level=error request_id=ops-1777815121120 method=POST path=/orders/ord-200/cancel user_id=user-2 expected_status=409 workflow=order_cancellation
