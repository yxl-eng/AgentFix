...
本文档用于规划 AgentFix 的第一版前端界面，当前仅定义产品定位、页面结构、交互重点和 MVP 范围，不涉及具体开发实现。待方案确认后，再进入前端架构设计和页面开发阶段。

## 设计目标

AgentFix 当前的核心能力不是内容展示，而是把一次自动修复流程完整呈现出来，包括事故输入、根因分析、补丁生成、验证执行、PR 产出和修复记录沉淀。
...
这是第一优先级页面，用于统一管理 AgentFix 所有读入参数。
...
目标是让用户一进入系统就能看到 AgentFix 的总体运行情况和最近修复状态。
...
- 先定义配置数据模型，与 `agentfix.yaml`、`agentfix.local.yaml`、环境变量映射对齐
...
- UI 配置保存的目标文件是仅写入 `agentfix.yaml`，还是区分 `agentfix.yaml` 与 `agentfix.local.yaml`
