...
如果你把密钥写在 `patchpilot.local.yaml`，正常直接运行 `patchpilot doctor` 即可。默认加载顺序是：

1. 读取 `patchpilot.yaml`
2. 如果存在 `patchpilot.local.yaml`，再用它覆盖同名配置

不要只执行 `patchpilot --config patchpilot.local.yaml doctor`，除非这个文件里包含完整配置；否则它不会自动合并 `patchpilot.yaml`。

`patchpilot doctor` 重点看这些字段：
...
这一步只测试 PatchPilot 自己的代码，不会创建 PR，也不会发飞书。
...
如果这里只失败在某个目标 demo 的业务测试，先确认你是在 PatchPilot 根目录运行，而不是进入了 `demo_services/fastapi-order-service` 目录。
...
完整链路需要 demo 服务本身是一个 GitHub 仓库，并且 `patchpilot.yaml` 里的 `repo_full_name` 指向真实仓库。
...
然后回到 PatchPilot 根目录，修改 `patchpilot.yaml`：
...
注意：`log_file` 是相对目标仓库 `repo_path` 的路径，不是相对 PatchPilot 根目录。

### 成功时应该看到什么

HTTP 返回里重点看：

- `status: fixed`：修复和验证通过；是否创建 PR 看 `pr_url`。
- `status: needs_human_verification`：语法/编译通过，但生成的回归测试没有通过，需要人工确认。
- `status: needs_manual_intervention`：模型补丁、配置、环境或验证没过，需要人工介入。
- `status: duplicate`：同一个 `incident_id` 已经处理过。

完整成功后会出现这些产物：

- GitHub 上出现一个 Draft PR。
- PatchPilot 仓库出现 `records/<incident_id>.json`。
- PatchPilot 仓库出现 `records/<incident_id>.md`。
- `.patchpilot-artifacts/` 下出现本次分析、补丁、验证、PR 的中间产物。
...
```powershell
Remove-Item .patchpilot-state\events.sqlite3
```
...
- `patchpilot doctor` 显示 target 和密钥配置正确。
- demo 服务原始 bug 能复现为 500。
- incident webhook 返回 `fixed`；如果配置了 GitHub remote 和 token，还应包含 `pr_url`。
...
- 飞书卡片包含目标服务、状态、根因摘要、修复摘要、修改文件、验证摘要和 PR/报告入口。
